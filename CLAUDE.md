...
- ✅ **Phase 4c (éditeur de paramètres)** : route `/budget` — partir d'une année officielle, modifier n'importe quel paramètre (montants, taux, seuils, **paliers**) et voir l'effet **officiel vs modifié**, écart par poste. `editeur-parametres.tsx` (Accordion générique sur le bundle), `parametres-meta.ts` (libellés). Le bundle modifiable = `structuredClone(PARAMETRES_OFFICIELS[base])`.
- Prochaines phases : déploiement ; comptes/sauvegarde de scénarios (Better Auth + Prisma + Postgres) ; extensions hors-MFQ ; couche IA. *(Note : libellés de champs de l'éditeur « humanisés » depuis les noms français des paramètres — pas encore traduits par champ.)*
