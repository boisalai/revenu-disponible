...
**Application Web.** Stack : Next.js 16 (App Router, **webpack** — `dev`/`build` avec `--webpack`), TypeScript strict, Tailwind 4, shadcn/ui sur Radix. Moteur dans `src/postes…` ; app dans `src/app` + `src/components` + `src/lib`, alias `@/* → src/*`. Le **calcul** reste **100 % client** ; un back-end (Better Auth + Prisma + Postgres) sert **uniquement** aux comptes et à la sauvegarde de scénarios (Phase 5). `npm run dev` sur le **port 3100** (3000 occupé par le site Docusaurus de l'utilisateur).
...
- Prochaines phases : extensions hors-MFQ ; couche IA. *(Note : libellés de champs de l'éditeur « humanisés » depuis les noms français des paramètres — pas encore traduits par champ.)*
