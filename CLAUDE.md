...
**Application Web (MVP, en cours).** Stack : Next.js 16 (App Router, **webpack** — `dev`/`build` avec `--webpack`), TypeScript strict, Tailwind 4, shadcn/ui sur Radix. Le moteur reste dans `src/postes…` ; l'app vit dans `src/app` (App Router) + `src/components` (`calculateur.tsx`) + `src/lib`, alias `@/* → src/*`. Le calcul est **100 % client** (aucune DB/auth/tRPC). `npm run dev` (port 3000, ou autre si occupé par le site Docusaurus de l'utilisateur). Fait : formulaire (5 situations, âges, revenus, enfants) + tableau de résultats **2025 vs 2026** ventilé par poste avec écart. Prochaines phases : volet pédagogique par poste (registre depuis `docs/revenu-disponible-pedagogie.md`), graphique des taux marginaux, comparaison de scénarios, couche IA. **Toujours lancer `npm test` + `npm run typecheck` ; le build se vérifie avec `npm run build`.**
