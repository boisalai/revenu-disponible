...
- ✅ **Activé en prod** : base **Neon** (`neondb`, AWS US East 1), variables Vercel (`DATABASE_URL` **pooled** `-pooler`, `BETTER_AUTH_SECRET`, `BETTER_AUTH_URL`), migration `init` appliquée. Comptes + sauvegarde **en ligne et vérifiés** (`get-session` → 200 ; `sign-in` interroge Neon → 401). Google OAuth : optionnel, non configuré (ajouter `GOOGLE_CLIENT_ID/SECRET` + `NEXT_PUBLIC_GOOGLE_ENABLED=true` + callback `…/api/auth/callback/google`).
