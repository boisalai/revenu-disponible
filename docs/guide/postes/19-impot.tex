...
\text{combiné} &= \mathrm{(seul)} \times M_{seul}
 + \mathrm{(65+)} \times M_{\text{âge}}
 + \text{(retraité)} \times \min(r,\, M_{pension}) \\
...
\begin{itemize}
  ...
  \item \emph{montant en raison de l'âge} (65~ans et plus) :
        $\pos{M_{\text{âge}} - 0{,}15 \times \pos{\text{revenu net} - s_{\text{âge}}}}$,
        avec $M_{\text{âge}} = \D{9028}$ et $s_{\text{âge}} = \D{45522}$ en 2025
...
Montant en raison de l'âge ($M_{\text{âge}}$)   & \D{3906}  & \D{3986} \\
...
Montant en raison de l'âge ($M_{\text{âge}}$)   & \D{9028}  & \D{9208} \\
...
\pnotes Deux mécanismes réels des couples ne sont pas modélisés.

(1) Le \emph{fractionnement des revenus de pension} : un conjoint peut
attribuer à l'autre jusqu'à \pc{50} de ses revenus de pension admissibles
--- rente d'un régime de pension agréé ; paiements d'un FERR et rente d'un
REER à compter de 65~ans ---, le cédant devant avoir 65~ans au Québec
(aucun âge minimal au fédéral pour la rente d'un RPA). La pension de la
Sécurité de la vieillesse, le SRG et la rente du RRQ ne sont \emph{pas}
fractionnables. Pour un couple de retraités aux pensions inégales (comme le
ménage type M11), le fractionnement réduirait l'impôt combiné --- le revenu
disponible affiché est donc prudent. Le calculateur officiel du MFQ ne le
modélise pas non plus : une seule source de revenu par adulte, sans
optimisation.

(2) Au Québec, le \emph{transfert entre conjoints} de la partie inutilisée
des crédits (que le modèle reproduit) exclut en réalité certains crédits
--- prolongation de carrière, intérêts sur prêt étudiant, notamment ---,
sans incidence ici puisque ces crédits ne sont pas modélisés.

\input{exemples/19-impot}
...
(abattement) ; Agence du revenu du Canada ; CFFP, \emph{Guide des mesures
fiscales}, fiches \og Crédit d'impôt personnel de base \fg{}, \og Transfert
de crédits entre conjoints \fg{} et \og Fractionnement des revenus de
pension et de retraite \fg{}\footnote{\url{https://cffp.recherche.usherbrooke.ca/outils-ressources/guide-mesures-fiscales/}}.
